...
* **Merger actor**: Overlay one IDS received on `F_INIT` on top of another and send the result out on `O_F`, for an actor that produces part of an IDS rather than all of it. ([documentation](https://imas-muscle3.readthedocs.io/en/latest/actor_merger.html))
