...
def core_profiles():
    ...
    for i in range(len(cp.time)):
        ...
        cp.profiles_1d[i].grid.rho_tor_norm = numpy.linspace(
            0.0, 1.0, 16
        )  # FLT_1D
        ...
        cp.profiles_1d[
            i
        ].electrons.temperature_fit.measured = temperature_fit_local
        cp.profiles_1d[
            i
        ].electrons.temperature_fit.local = temperature_fit_local
